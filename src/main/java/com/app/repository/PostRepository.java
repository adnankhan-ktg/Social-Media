...
public interface PostRepository extends JpaRepository<Post, Integer> {
    ...
    @Query(value = "WITH LatestUserPosts AS (" +
            "    SELECT" +
            "        p.user_id," +
            "        p.id AS post_id," +
            "        p.content_type," +
            ...
            "        post AS p" +
            "    WHERE" +
            "        p.user_id IN (SELECT followed_user_id FROM user_followings WHERE follower_user_id = 1 AND status = 'APPROVED')" +
            "    AND (" +
            "        p.id NOT IN (SELECT post_id FROM interaction_log WHERE user_id = 1)" +
            "        OR p.id IS NULL" +
            "    )" +
            "    ORDER BY" +
            "        p.timestamp DESC" +
            ...
            "        p.timestamp,"
            +
            "        bu.business_name AS business_name  -- Replace with the actual business name field"
            +
            "    FROM"
            +
            "        post AS p INNER JOIN business_user bu ON bu.id = p.business_user_id"
            +
            "    WHERE"
            +
            ...
            +
            "LEFT JOIN LatestUserPosts AS lp"
            +
            "ON f.id = lp.user_id"
            +
            "WHERE"
            ...
            +
            "FROM LatestBusinessPost AS lb", nativeQuery = true)
